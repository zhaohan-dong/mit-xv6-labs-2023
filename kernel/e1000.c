...
int e1000_transmit(struct mbuf *m)
{
    ...
    acquire(&e1000_lock);
    if ((E1000_TXD_STAT_DD & tx_ring[tx_ring_index].status) == 0)
    {
    ...
    // Free transmitted ring buffer
    struct mbuf *b = tx_mbufs[tx_ring_index];
    if (b)
    {
        mbuffree(b);
    }
...
static void e1000_recv(void)
{
    ...
    // See 3.2.6 on E1000 manual about E1000_RDT
    int rx_ring_index = (regs[E1000_RDT] + 1) % RX_RING_SIZE;

    acquire(&e1000_lock);

    // If a packet is available to receive
    while ((E1000_RXD_STAT_DD & rx_ring[rx_ring_index].status) != 0)
    {
        ...
        // Error if unable to allocate new buffer
        rx_mbufs[rx_ring_index] = mbufalloc(0);
        if (!rx_mbufs[rx_ring_index])
            panic("e1000");

        // Set address in descriptor to mbufs->head
